...
<?php 
if($_GET['operacion']=='asignar'){
	include('clases/bus.php');
	$objBus=new Bus();
	$bus=split("-",$_POST["cboBus"]);
	$empleado=split("-",$_POST["cboEmpleado"]);
	$objBus->asignarConductor($empleado[0],$bus[0],$objConexion);
}	

if(!$_GET['operacion']){?>
<p>Seleccione el bus y luego seleccione el empleado que lo conducira:</p>
    <form id="form1" name="form1" method="post" action="asignar_conductor.php?operacion=asignar">
      
        <table width="90%" border="1" cellspacing="0" cellpadding="10px" align="center" >
          <tr>
            <td width="32%">Unidad de Transporte:</td>
            <td width="68%">
            <select name="cboBus">
<?php 
			$resultadoBuses=$objConexion->ejecutarComando("
			SELECT TBL_MG_UNIDADES_TRANSPORTE.CODIGO_UNIDAD_TRANSPORTE,
			TBL_MG_UNIDADES_TRANSPORTE.NUMERO_PLACA FROM TBL_MG_UNIDADES_TRANSPORTE;");
			while($buses=mysql_fetch_array($resultadoBuses)){
				echo "<option>".$buses["CODIGO_UNIDAD_TRANSPORTE"]."-(Placa:".$buses["NUMERO_PLACA"].")</option>";
			}	
			mysql_free_result($resultadoBuses);
?>
          ...
          <tr>
            <td>Conductor:</td>
            <td><select name="cboEmpleado">
<?php 
$resultadoEmpleados=$objConexion->ejecutarComando("SELECT TBL_MG_EMPLEADOS.CODIGO_EMPLEADO, 
	TBL_MG_EMPLEADOS.NOMBRES, TBL_MG_EMPLEADOS.APELLIDOS FROM TBL_MG_EMPLEADOS
	WHERE (((TBL_MG_EMPLEADOS.CODIGO_CARGO)=1));");
			while($empleados=mysql_fetch_array($resultadoEmpleados)){
				echo "<option>".$empleados['CODIGO_EMPLEADO']."-".$empleados['NOMBRES']." ".$empleados['APELLIDOS']."</option>";
			}	
			mysql_free_result($resultadoEmpleados);

?>
			<option selected="selected"></option>
            </select></td>
          </tr>
          <tr><td colspan="2"><input type="submit" value="Asignar Conductor" /></td></tr>
        </table>
    </form>
    <?php } 
